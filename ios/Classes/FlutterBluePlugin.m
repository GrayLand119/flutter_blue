...
#ifdef DEBUG
#   define GLLog(fmt, ...) NSLog((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ## __VA_ARGS__);
#else
#   define GLLog(...)
#endif
...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    if ([@"setLogLevel" isEqualToString:call.method]) {
        ...
        result(data);
        [self makeSureCallbackBLEState:5];
...
- (void)centralManagerDidUpdateState:(nonnull CBCentralManager *)central {
    GLLog(@"centralManagerDidUpdateState: %@ - %@", @(central.state), @(_centralManager.state));
...
#pragma mark - GrayLand119

- (instancetype)init
{
    self = [super init];
    if (self) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_UIApplicationDidBecomeActiveNotification) name:UIApplicationDidFinishLaunchingNotification object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_UIApplicationDidBecomeActiveNotification) name:UIApplicationDidBecomeActiveNotification object:nil];
    }
    return self;
}

- (void)_UIApplicationDidFinishLaunchingNotification {
    if (_stateStreamHandler.sink != nil) {
        FlutterStandardTypedData *data = [self toFlutterData:[self toBluetoothStateProto:self->_centralManager.state]];
        self.stateStreamHandler.sink(data);
    }
...
- (void)makeSureCallbackBLEState:(NSInteger)retry {
    GLLog(@"_stateStreamHandler.sink: %@", _stateStreamHandler.sink);
    ...
    if (_stateStreamHandler.sink != nil) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self makeSureCallbackBLEState:retry - 1];
        });
    }
}
